    ...
    >
      ...
      >
        <article className="bg-white rounded-2xl p-4 sm:p-6 flex flex-col h-full shadow-md hover:shadow-lg transition-all duration-300 border border-gray-100">
          ...
          <div className="relative h-40 sm:h-48 flex items-center justify-center mb-4 sm:mb-6 overflow-hidden rounded-lg bg-gray-50">
            {imageUrl ? (
              <Image
                src={imageUrl}
                ...
                priority={priority}
                sizes={sizes}
                className={`object-cover transition-transform duration-300 group-hover:scale-[${HOVER_SCALE}]`}
                onError={(e) => {
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <p className="text-gray-600 text-xs sm:text-sm line-clamp-2 mb-2 sm:mb-3 min-h-[2rem]">
                {product.shortDescription || DEFAULT_DESCRIPTION}
              </p>

              {/* Certifications */}
              {certificationBadges.length > 0 && (
                <div className="flex flex-wrap gap-1 sm:gap-2 mb-2 sm:mb-3" role="list" aria-label="Product certifications">
